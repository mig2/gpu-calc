...
export function InferenceCalculator() {
  ...
  function copyMarkdown() {
    const lines = [
      ...
      '',
      `**Usage:** ${requestsPerDay.toLocaleString()} req/day × ${avgInputTokens} input + ${avgOutputTokens} output tokens`,
      `**Daily volume:** ${((requestsPerDay * (avgInputTokens + avgOutputTokens)) / 1e6).toFixed(1)}M tokens/day`,
      '',
      '## API (Buy)',
      `- **${apiResult?.provider} ${apiResult?.model}**: $${apiResult?.monthlyCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}/month ($${apiResult?.costPerRequest.toFixed(4)}/req)`,
      '',
      '## Self-Host (Build)',
      `- **${selfHostResult?.model}** on ${selfHostResult?.gpuCount}× GPU (${selfHostResult?.cloudProvider}): $${selfHostResult?.monthlyGpuCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}/month`,
      `- Throughput: ${selfHostResult?.maxOutputTokensPerSec} tok/s (${selfHostResult?.utilizationPercent.toFixed(1)}% utilized)`,
      '',
      '## Breakeven',
      breakeven?.apiCheaperBelow
        ...
        : `Self-hosting saves $${Math.abs(breakeven?.monthlySavingsAtCurrentVolume ?? 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}/month at current volume.`,
    ]
  ...
  return (
    <div className="inference-calculator">
      <div className="export-controls" style={{ marginBottom: '1rem' }}>
